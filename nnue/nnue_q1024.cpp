...
static inline void matvec_128x1024(const int16_t* __restrict__ a, const int16_t* __restrict__ b, int16_t* __restrict__ c, 
...
static inline void matvec_64x128(const int16_t* __restrict__ a, const int16_t* __restrict__ b, int16_t* __restrict__ c,
                                const int32_t* __restrict__ bias) {    
    constexpr int register_width = 256 / 16;    
    constexpr int m = 64;
    constexpr int n = 128;
...
static inline void matvec_3x64(const int16_t* __restrict__ a, const int16_t* __restrict__ b, int32_t* __restrict__ c,
                                const int32_t* __restrict__ bias) {
    constexpr int register_width = 256 / 16;    
    constexpr int n = 64;
    const int num_in_chunks = n / register_width;
...
NNUE_Q1024::Accumulator NNUE_Q1024::make_accumulator() const {
    Accumulator a;
...
std::tuple<float, float, float> NNUE_Q1024::output(Accumulator& a) {        
    ...
    matvec_128x1024(h1_to_h2, h1_16, h2_16, h2_bias);    
    matvec_64x128(h2_to_h3, h2_16, h3_16, h3_bias);
    matvec_3x64(h3_to_output, h3_16, output, output_bias);    
...
void NNUE_Q1024::load(const std::string& filename) {
...
void NNUE_Q1024::init(const Yolah& yolah, Accumulator& a) {
    for (int i = 0; i < H1_SIZE; i++) {
        a.acc[i] = h1_bias[i];       
    }
...
void NNUE_Q1024::play(uint8_t player, const Move& m, Accumulator& a) {
...
void NNUE_Q1024::undo(uint8_t player, const Move& m, Accumulator& a) {
    int from = 63 - m.from_sq();
    int to = 63 - m.to_sq();
    // black positions + white positions + empty positions
    int pos = (player == Yolah::BLACK) ? 0 : 64;    
...
int main(int argc, char* argv[]) {
    ...
    NNUE_Q1024 nnue;
    try {
        nnue.load("nnue_q_1024x128x64x3.txt");
    } catch (const char* e) {
        cout << e << endl;
        exit(EXIT_FAILURE);
    }    
