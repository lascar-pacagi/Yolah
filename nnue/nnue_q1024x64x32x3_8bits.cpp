#include "nnue_q1024x64x32x3_8bits.h"
#include <fstream>
#include <string>
#include <regex>
#include "game.h"
...
static inline void matvec_64x1024_slow(const int8_t* __restrict__ a, const int8_t* __restrict__ b, int8_t* __restrict__ c, 
    const int16_t* __restrict__ bias) {
        constexpr int m = 64;
        constexpr int n = 1024;
        for (int i = 0; i < m; i++) {
            int16_t sum = 0;
            for (int j = 0; j < n; j++) {
                sum += (int16_t)a[i * n + j] * (int16_t)b[j]; 
            } 
            sum += bias[i];           
            sum >>= SHIFT;            
            c[i] = std::max(int16_t(0), std::min(FACTOR, sum));
            std::cout << (int)c[i] << ' ';
        }
        std::cout << std::endl;
}
...
static inline void matvec_32x64_slow(const int8_t* __restrict__ a, const int8_t* __restrict__ b, int8_t* __restrict__ c,
    const int16_t* __restrict__ bias) {
        constexpr int m = 32;
        constexpr int n = 64;
        for (int i = 0; i < m; i++) {
...
static inline void matvec_3x32_slow(const int8_t* __restrict__ a, const int8_t* __restrict__ b, int32_t* __restrict__ c,
    const int16_t* __restrict__ bias) {
        constexpr int m = 3;
        constexpr int n = 32;
        for (int i = 0; i < m; i++) {
            int32_t sum = 0;
            for (int j = 0; j < n; j++) {
                sum += (int32_t)a[i * n + j] * (int32_t)b[j];
            }
            c[i] = sum + bias[i];
            c[i] >>= SHIFT;
            std::cout << c[i] << ' ';
...
NNUE_Q1024x64x32x3_8bits::Accumulator NNUE_Q1024x64x32x3_8bits::make_accumulator() const {
    Accumulator a;
...
std::tuple<float, float, float> NNUE_Q1024x64x32x3_8bits::output(Accumulator& a) {        
...
void NNUE_Q1024x64x32x3_8bits::load(const std::string& filename) {
...
void NNUE_Q1024x64x32x3_8bits::init(const Yolah& yolah, Accumulator& a) {
    for (int i = 0; i < H1_SIZE; i++) {
        a.acc[i] = h1_bias[i];       
    }
...
void NNUE_Q1024x64x32x3_8bits::play(uint8_t player, const Move& m, Accumulator& a) {
...
void NNUE_Q1024x64x32x3_8bits::undo(uint8_t player, const Move& m, Accumulator& a) {
    int from = 63 - m.from_sq();
    int to = 63 - m.to_sq();
    // black positions + white positions + empty positions
    int pos = (player == Yolah::BLACK) ? 0 : 64;    
