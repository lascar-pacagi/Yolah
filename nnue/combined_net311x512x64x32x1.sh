#!/bin/bash
# ────────────────────────────────────────────────────────────────────────────
# combined_net311x512x64x32x1.sh — submit with `sbatch combined_net311x512x64x32x1.sh`
#
# Pipeline:
...
#   2. Train combined_net311x512x64x32x1.py via DDP across all visible GPUs.
...
#SBATCH --job-name=combined311
#SBATCH --output=combined311_%j.out
#SBATCH --error=combined311_%j.err
#SBATCH --gres=gpu:1
...
SIF="${SIF:-${SLURM_SUBMIT_DIR:-$PWD}/combined_net311x512x64x32x1.sif}"
...
echo "[$(date '+%F %T')] === Training: combined_net311x512x64x32x1.py ==="
singularity exec --nv \
    --bind "${CACHE_DIR}:/cache" \
    --bind "${MODEL_DIR}:/mnt" \
    ...
    "${SIF}" \
    bash -c "cd /nnue && python3 combined_net311x512x64x32x1.py"
