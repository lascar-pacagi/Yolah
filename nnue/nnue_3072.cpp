...
static void matvec_3072x16(const float* __restrict__ a, const float* __restrict__ b, float* __restrict__ c) {    
    vec8 sum[2]{};
    constexpr int m = 3072;
    constexpr int n = 16;
    for (int i = 0; i < m; i++) {
        vec8 bb = vec8{} + b[i];
...
static inline void matvec_16x32(const float* __restrict__ a, const float* __restrict__ b, float* __restrict__ c) {    
    vec8 sum[4]{};
    constexpr int m = 16;
    constexpr int n = 32;
    for (int i = 0; i < m; i++) {
...
NNUE_3072::Accumulator NNUE_3072::make_accumulator() const {
    Accumulator a;
...
std::tuple<float, float, float> NNUE_3072::output(Accumulator& a) {    
    float h1[H1_SIZE];
    float h2[H2_SIZE];
    float h3[H3_SIZE];
...
void NNUE_3072::load(const std::string& filename) {
...
void NNUE_3072::init(const Yolah& yolah, Accumulator& a) {
...
void NNUE_3072::play(uint8_t player, const Move& m, Accumulator& a) {
...
void NNUE_3072::undo(uint8_t player, const Move& m, Accumulator& a) {
    int from = 63 - m.from_sq();
    int to = 63 - m.to_sq();
    // black positions + white positions + empty positions
    int pos = (player == Yolah::BLACK) ? 0 : 64;    
...
std::pair<float, float> NNUE_3072::minmax_weights() const {
...
std::pair<float, float> NNUE_3072::percentile_weights(float percentile) const {
...
void NNUE_3072::get_activations(Accumulator& a, std::vector<float>& activations) {
    float h1[H1_SIZE];
    float h2[H2_SIZE];
    float h3[H3_SIZE];
    float output[OUTPUT_SIZE];
...
std::pair<float, float> NNUE_3072::minmax_activations(const std::string& filename) {
    std::ifstream ifs(filename, std::ifstream::in);
...
std::pair<float, float> NNUE_3072::percentile_activations(const std::string& filename, float percentile) {
    std::ifstream ifs(filename, std::ifstream::in);
    std::regex re_moves(R"(((\w\d):(\w\d))+)", std::regex_constants::ECMAScript);
    Accumulator acc = make_accumulator();
    std::vector<float> activations;
