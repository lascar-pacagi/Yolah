...
static constexpr int32_t FACTOR = 64;
static constexpr int SHIFT = 6;
static constexpr int A = 1;

static inline void matvec_128(int n, const int32_t* __restrict__ a, const int32_t* __restrict__ b, int32_t* __restrict__ c) {    
    for (int i = 0; i < 128; i++) {
        int32_t sum = 0;
        for (int j = 0; j < n; j++) {
            sum += a[i * n + j] * b[j];
...
static inline void matvec3x64(const int32_t* __restrict__ a, const int32_t* __restrict__ b, int32_t* __restrict__ c) {
    for (int i = 0; i < 3; i++) {
        int32_t sum = 0;
        for (int j = 0; j < 64; j++) {
...
static inline void clamp(int n, int32_t* input, int32_t* output) {
    for (int i = 0; i < n; i++) {
...
NNUE_Q4::Accumulator NNUE_Q4::make_accumulator() const {
    Accumulator a;
...
std::tuple<float, float, float> NNUE_Q4::output(Accumulator& a) {        
    int32_t h2[H2_SIZE];
    int32_t h3[H3_SIZE];
    int32_t output[OUTPUT_SIZE];
    int32_t h1[H1_SIZE];
    int32_t h2_8[H2_SIZE];
    int32_t h3_8[H3_SIZE];
    for (int i = 0; i < H1_SIZE; i++) {
        int32_t v = a.acc[i];
...
void NNUE_Q4::load(const std::string& filename) {
    std::ifstream ifs(filename, std::ifstream::in);
    read_matrix<int32_t, H1_SIZE, INPUT_SIZE, TRANSPOSE>(ifs, input_to_h1);
    read_bias<int32_t, H1_SIZE>(ifs, h1_bias);
    read_matrix<int32_t, H2_SIZE, H1_SIZE>(ifs, h1_to_h2);
    read_bias<int32_t, H2_SIZE>(ifs, h2_bias);
    read_matrix<int32_t, H3_SIZE, H2_SIZE>(ifs, h2_to_h3);
    read_bias<int32_t, H3_SIZE>(ifs, h3_bias);
    read_matrix<int32_t, OUTPUT_SIZE, H3_SIZE>(ifs, h3_to_output);
...
void NNUE_Q4::init(const Yolah& yolah, Accumulator& a) {
    for (int i = 0; i < H1_SIZE; i++) {
        a.acc[i] = h1_bias[i];       
    }
    ...
    if (yolah.current_player() == Yolah::WHITE) {
        const int32_t* turn = input_to_h1 + (INPUT_SIZE - 1) * H1_SIZE;
        for (int i = 0; i < H1_SIZE; i++) {
            a.acc[i] += turn[i];
        }
    }
...
void NNUE_Q4::play(uint8_t player, const Move& m, Accumulator& a) {
...
void NNUE_Q4::undo(uint8_t player, const Move& m, Accumulator& a) {
    int from = 63 - m.from_sq();
    int to = 63 - m.to_sq();
    // black positions + white positions + empty positions
    int pos = (player == Yolah::BLACK) ? 0 : 64;    
