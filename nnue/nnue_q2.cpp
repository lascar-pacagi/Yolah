...
static inline void relu(int n, int32_t* input, int8_t* output) {
    for (int i = 0; i < n; i++) {
        output[i] = std::min(std::max(0, input[i]), 127);
    }
}
...
NNUE_Q2::Accumulator NNUE_Q2::make_accumulator() const {
    Accumulator a;
...
std::tuple<float, float, float> NNUE_Q2::output(Accumulator& a) {        
    int32_t h2[H2_SIZE];
    int32_t h3[H3_SIZE];
    ...
    // }
    // std::cout << '\n' << "#################\n";
    matvec(H2_SIZE, h2_to_h3, h2_8, h3);
...
void NNUE_Q2::load(const std::string& filename) {
    ...
    for (int i = 0; i < 64; i++) {
        ...
        for (int j = 0; j < H1_SIZE; j++) {
            turn_white[j] += input_to_h1[row + j] * A;
        }
    }
...
void NNUE_Q2::init(const Yolah& yolah, Accumulator& a) {
    for (int i = 0; i < H1_SIZE; i++) {
        a.acc[i] = h1_bias[i];       
    }
...
void NNUE_Q2::play(uint8_t player, const Move& m, Accumulator& a) {
    ...
    for (int j = 0; j < H1_SIZE; j++) {
        ...
        int32_t v4 = input_to_h1[occupied_offset + j] * A;
        int32_t v5 = -input_to_h1[free_offset + j] * A;
...
void NNUE_Q2::undo(uint8_t player, const Move& m, Accumulator& a) {
    int from = 63 - m.from_sq();
    int to = 63 - m.to_sq();
    // black positions + white positions + empty positions + occupied positions + free positions
    int pos = (player == Yolah::BLACK) ? 0 : 64;    
