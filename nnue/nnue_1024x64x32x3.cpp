...
static void matvec_1024x64(const float* __restrict__ a, const float* __restrict__ b, float* __restrict__ c) {    
    vec8 sum[8]{};
...
static inline void matvec_64x32(const float* __restrict__ a, const float* __restrict__ b, float* __restrict__ c) {    
    vec8 sum[4]{};
    constexpr int m = 64;
    constexpr int n = 32;
    for (int i = 0; i < m; i++) {
...
NNUE_1024x64x32x3::NNUE_1024x64x32x3() {
...
NNUE_1024x64x32x3::Accumulator NNUE_1024x64x32x3::make_accumulator() const {
    Accumulator a;
...
std::tuple<float, float, float> NNUE_1024x64x32x3::output(Accumulator& a) {    
    float h1[H1_SIZE];
    float h2[H2_SIZE];
    float h3[H3_SIZE];
    ...
    //std::cout << '\n' << "#################\n";
    matvec_1024x64(weights_and_biases + H1_TO_H2, h1, h2);
    ...
    // }
    // std::cout << '\n' << "#################\n";
    matvec_64x32(weights_and_biases + H2_TO_H3, h2, h3);
...
void NNUE_1024x64x32x3::load(const std::string& filename) {
...
void NNUE_1024x64x32x3::save_quantized(const std::string& filename, float scale) {
...
void NNUE_1024x64x32x3::init(const Yolah& yolah, Accumulator& a) {
...
void NNUE_1024x64x32x3::play(uint8_t player, const Move& m, Accumulator& a) {
...
void NNUE_1024x64x32x3::undo(uint8_t player, const Move& m, Accumulator& a) {
    int from = 63 - m.from_sq();
    int to = 63 - m.to_sq();
    // black positions + white positions + empty positions
    int pos = (player == Yolah::BLACK) ? 0 : 64;    
...
std::pair<float, float> NNUE_1024x64x32x3::minmax_weights() const {
...
std::pair<float, float> NNUE_1024x64x32x3::percentile_weights(float percentile) const {
...
void NNUE_1024x64x32x3::get_activations(Accumulator& a, std::vector<float>& activations) {
    float h1[H1_SIZE];
    float h2[H2_SIZE];
    float h3[H3_SIZE];
    float output[OUTPUT_SIZE];
...
std::pair<float, float> NNUE_1024x64x32x3::minmax_activations(const std::string& filename) {
    std::ifstream ifs(filename, std::ifstream::in);
...
std::pair<float, float> NNUE_1024x64x32x3::percentile_activations(const std::string& filename, float percentile) {
    std::ifstream ifs(filename, std::ifstream::in);
    std::regex re_moves(R"(((\w\d):(\w\d))+)", std::regex_constants::ECMAScript);
    Accumulator acc = make_accumulator();
    std::vector<float> activations;
...
//     nnue.load("nnue_1024x64x32x3.25.txt");
//     //nnue.save_quantized("nnue_q_1024x64x32x3.0.txt", 64);
//     nnue.save_quantized("nnue_q_1024x64x32x3.25.txt", 64);
//     //return 0;
