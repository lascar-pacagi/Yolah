...
class GameDataset(Dataset):
    ...
    def encode_yolah(yolah):
        # Per-position certain-win signal. Scores are live on `yolah`
        # after replaying the move prefix, so no label leakage.
        #
        # Yolah alternates plies and "cannot move" is permanent, so the
        # normal alternating-play delta is 0 when black is to move and
        # +1 when white is to move. Any deviation from that baseline
        # implies the other side has already passed (is permanently
        # stuck) and the remaining game is played solo.
        #
        # Fold turn into an "effective lead" so the certain-win
        # threshold is ±1 regardless of whose turn it is:
        #   effective_delta = delta           if black to move
        #                   = delta - 1       if white to move
        #   effective_delta >= 1   => black certain-win
        #   effective_delta <= -1  => white certain-win
        delta = yolah.black_score - yolah.white_score
        black_to_move = (yolah.nb_plies() & 1) == 0
        effective_delta = delta - (0 if black_to_move else 1)
        diff_score = [1.0 if effective_delta >= 1 else 0.0,
                      1.0 if effective_delta <= -1 else 0.0]
...
# black positions + white positions + empty positions + turn + diff score
