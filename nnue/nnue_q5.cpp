...
static constexpr int8_t FACTOR = 64;
...
NNUE_Q5::Accumulator NNUE_Q5::make_accumulator() const {
    Accumulator a;
...
std::tuple<float, float, float> NNUE_Q5::output(Accumulator& a) {        
    int32_t h2[H2_SIZE];
    int32_t h3[H3_SIZE];
...
void NNUE_Q5::load(const std::string& filename) {
...
void NNUE_Q5::init(const Yolah& yolah, Accumulator& a) {
    for (int i = 0; i < H1_SIZE; i++) {
        a.acc[i] = h1_bias[i];       
    }
...
void NNUE_Q5::play(uint8_t player, const Move& m, Accumulator& a) {
...
void NNUE_Q5::undo(uint8_t player, const Move& m, Accumulator& a) {
    int from = 63 - m.from_sq();
    int to = 63 - m.to_sq();
    // black positions + white positions + empty positions
    int pos = (player == Yolah::BLACK) ? 0 : 64;    
