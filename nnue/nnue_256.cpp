...
static void matvec_256x256(const float* __restrict__ a, const float* __restrict__ b, float* __restrict__ c) {    
    vec8 sum[32]{};
    constexpr int m = 256;
    constexpr int n = 256;
    for (int i = 0; i < m; i++) {
...
static inline void matvec_256x32(const float* __restrict__ a, const float* __restrict__ b, float* __restrict__ c) {    
    vec8 sum[4]{};
    constexpr int m = 256;
    constexpr int n = 32;
    for (int i = 0; i < m; i++) {
...
NNUE_256::Accumulator NNUE_256::make_accumulator() const {
    Accumulator a;
...
std::tuple<float, float, float> NNUE_256::output(Accumulator& a) {    
    float h1[H1_SIZE];
    float h2[H2_SIZE];
    float h3[H3_SIZE];
    ...
    //std::cout << '\n' << "#################\n";
    matvec_256x256(weights_and_biases + H1_TO_H2, h1, h2);
    ...
    // }
    // std::cout << '\n' << "#################\n";
    matvec_256x32(weights_and_biases + H2_TO_H3, h2, h3);
...
void NNUE_256::load(const std::string& filename) {
...
void NNUE_256::save_quantized(const std::string& filename, float scale) {
...
void NNUE_256::init(const Yolah& yolah, Accumulator& a) {
...
void NNUE_256::play(uint8_t player, const Move& m, Accumulator& a) {
...
void NNUE_256::undo(uint8_t player, const Move& m, Accumulator& a) {
    int from = 63 - m.from_sq();
    int to = 63 - m.to_sq();
    // black positions + white positions + empty positions
    int pos = (player == Yolah::BLACK) ? 0 : 64;    
...
std::pair<float, float> NNUE_256::minmax_weights() const {
...
std::pair<float, float> NNUE_256::percentile_weights(float percentile) const {
...
void NNUE_256::get_activations(Accumulator& a, std::vector<float>& activations) {
    float h1[H1_SIZE];
    float h2[H2_SIZE];
    float h3[H3_SIZE];
    float output[OUTPUT_SIZE];
...
std::pair<float, float> NNUE_256::minmax_activations(const std::string& filename) {
    std::ifstream ifs(filename, std::ifstream::in);
...
std::pair<float, float> NNUE_256::percentile_activations(const std::string& filename, float percentile) {
    std::ifstream ifs(filename, std::ifstream::in);
    std::regex re_moves(R"(((\w\d):(\w\d))+)", std::regex_constants::ECMAScript);
    Accumulator acc = make_accumulator();
    std::vector<float> activations;
...
int main(int argc, char* argv[]) {
    using namespace std;
    NNUE_256 nnue;
    nnue.load("nnue_256x256x32x3.0.txt");
    nnue.save_quantized("nnue_q_256x256x32x3.txt", 64);
    //return 0;
