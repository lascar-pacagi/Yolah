"""
combined_net311x512x64x32x1.py — multi-GPU training of the COMBINED network
((193 nnue board-input ⊕ 118 features, turn-deduplicated) → 512 → 64 → 32 → 1)
with an AlphaZero-style scalar value head.

Rationale
─────────
The 193-input is the raw, lossless board (black/white/empty bitboards + turn).
The 119 features are hand-crafted, lossy summaries (mobility, blocked rays, …).
Concatenating them is a "wide & deep" bet: a *smaller* body (L1 = 512 vs the
pure-nnue 1024) gets the raw board AND a head-start from engineered features.
Param count ≈ 195k vs the pure-nnue ≈ 266k. NOTE: this trades away NNUE's
incremental-update speed — the 119 features do not update cheaply per move — so
it is aimed at eval QUALITY, not pure-nnue node rates.
...
class Net(nn.Module):
    ...
    """
    def __init__(self, input_size=INPUT_SIZE, l1_size=512, l2_size=64, l3_size=32):
        super().__init__()
...
MODEL_PATH = "/mnt/"
MODEL_NAME = "combined_311x512x64x32x1"
