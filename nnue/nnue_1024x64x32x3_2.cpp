...
NNUE_1024x64x32x3_2::NNUE_1024x64x32x3_2() {
...
NNUE_1024x64x32x3_2::Accumulator NNUE_1024x64x32x3_2::make_accumulator() const {
    Accumulator a;
...
static inline void matvec_64x1024(const float* __restrict__ a, const float* __restrict__ b, float* __restrict__ c, 
...
static inline void matvec_32x64(const float* __restrict__ a, const float* __restrict__ b, float* __restrict__ c,
                                const float* __restrict__ bias) {    
    constexpr int register_width = 256 / 32;    
    constexpr int m = 32;
    constexpr int n = 64;
...
std::tuple<float, float, float> NNUE_1024x64x32x3_2::output(Accumulator& a) {        
    ...
    //std::cout << std::endl;
    matvec_64x1024(weights_and_biases + H1_TO_H2, h1, h2, weights_and_biases + H2_BIAS);
    // for (int i = 0; i < H2_SIZE; i++) {
    //     std::cout << (int)h2[i] << ' ';
    // }
    //std::cout << std::endl;
    matvec_32x64(weights_and_biases + H2_TO_H3, h2, h3, weights_and_biases + H3_BIAS);
...
void NNUE_1024x64x32x3_2::load(const std::string& filename) {
...
void NNUE_1024x64x32x3_2::save_quantized(const std::string& filename, float scale) {
...
void NNUE_1024x64x32x3_2::init(const Yolah& yolah, Accumulator& a) {
...
void NNUE_1024x64x32x3_2::play(uint8_t player, const Move& m, Accumulator& a) {
...
void NNUE_1024x64x32x3_2::undo(uint8_t player, const Move& m, Accumulator& a) {
    int from = 63 - m.from_sq();
    int to = 63 - m.to_sq();
    // black positions + white positions + empty positions
    int pos = (player == Yolah::BLACK) ? 0 : 64;    
...
//     using namespace std;
//     NNUE_1024x64x32x3_2 nnue;
//     nnue.load("nnue_1024x64x32x3.20.txt");
//     //nnue.save_quantized("nnue_q_1024x64x32x3.20.txt", 64);
//     //return 0;
