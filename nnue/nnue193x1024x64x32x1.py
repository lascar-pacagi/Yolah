...
─────────────────────────────────────────────────────
The cache produced by preprocess_nnue.py is a flat memmap of N records:
    inputs.u8  : (N, 193) uint8   — board + turn (binary 0/1 bytes)
    values.i8  : (N,)     int8    — z ∈ {-1, 0, +1} from current player's POV

A standard DataLoader samples positions at uniformly random indices, which on
an HDD-backed cache collapses into millions of random ~193-byte reads. This
script instead uses a `ChunkedShuffleLoader`:
  • one background producer thread per rank reads a CONTIGUOUS chunk
    (~800 MB) sequentially into RAM, shuffles WITHIN the chunk, slices it into
    batches, pins each batch, and pushes onto a bounded queue;
  ...
  • DDP is configured with `gradient_as_bucket_view=True` and
    `static_graph=True` to avoid the periodic ~50 s NCCL all-reduce stall that
    plagued the DataLoader variant on multi-GPU runs.

The model has NNUE-style clamp([0,1]) activations on fc1–fc3 and clamps those
layers' weights to ±127/64 each iteration so the trained body stays
quantizable for int8 C++ inference. The value head (fc4 → tanh) is left
unclamped.
"""
...
# black bits + white bits + empty bits + turn byte
INPUT_SIZE = 64 + 64 + 64 + 1                # 193
...
#     • at 193 bytes/position it is ~800 MB of uint8 — small enough for a
...
# stay ahead of the GPU. Each NNUE batch (bs=512) is ~400 KB float32 X + 2 KB
# y → 32 batches ≈ 13 MB of pinned RAM per rank: negligible.
BATCH_QUEUE_DEPTH = 32


class ChunkedShuffleLoader:
    """
    Streaming data loader over the (inputs.u8, values.i8) memmap pair.
    ...
    def _open(self):
        ...
        if n_inputs_cols != INPUT_SIZE:
            raise ValueError(
                f"Cache input width {n_inputs_cols} != expected {INPUT_SIZE}. "
                "Re-run preprocess_nnue.py.")
