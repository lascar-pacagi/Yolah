...
static inline void matvec_256x256(const int16_t* __restrict__ a, const int16_t* __restrict__ b, int16_t* __restrict__ c, 
...
static inline void matvec_32x256(const int16_t* __restrict__ a, const int16_t* __restrict__ b, int16_t* __restrict__ c,
                                const int32_t* __restrict__ bias) {    
    constexpr int register_width = 256 / 16;    
    constexpr int m = 32;
    constexpr int n = 256;
...
NNUE_Q256::Accumulator NNUE_Q256::make_accumulator() const {
    Accumulator a;
...
std::tuple<float, float, float> NNUE_Q256::output(Accumulator& a) {        
    ...
    matvec_256x256(h1_to_h2, h1_16, h2_16, h2_bias);    
    matvec_32x256(h2_to_h3, h2_16, h3_16, h3_bias);
...
void NNUE_Q256::load(const std::string& filename) {
...
void NNUE_Q256::init(const Yolah& yolah, Accumulator& a) {
    for (int i = 0; i < H1_SIZE; i++) {
        a.acc[i] = h1_bias[i];       
    }
...
void NNUE_Q256::play(uint8_t player, const Move& m, Accumulator& a) {
...
void NNUE_Q256::undo(uint8_t player, const Move& m, Accumulator& a) {
    int from = 63 - m.from_sq();
    int to = 63 - m.to_sq();
    // black positions + white positions + empty positions
    int pos = (player == Yolah::BLACK) ? 0 : 64;    
...
int main(int argc, char* argv[]) {
    ...
    NNUE_Q256 nnue;
    try {
        nnue.load("nnue_q_256x256x32x3.txt");
    } catch (const char* e) {
        cout << e << endl;
        exit(EXIT_FAILURE);
    }    
