...
static void matvec_512x64(const float* __restrict__ a, const float* __restrict__ b, float* __restrict__ c) {    
    vec8 sum[8]{};
    constexpr int m = 512;
    constexpr int n = 64;
    for (int i = 0; i < m; i++) {
...
NNUE_512::Accumulator NNUE_512::make_accumulator() const {
    Accumulator a;
...
std::tuple<float, float, float> NNUE_512::output(Accumulator& a) {    
    float h1[H1_SIZE];
    float h2[H2_SIZE];
    float h3[H3_SIZE];
    ...
    //std::cout << '\n' << "#################\n";
    matvec_512x64(weights_and_biases + H1_TO_H2, h1, h2);
...
void NNUE_512::load(const std::string& filename) {
...
void NNUE_512::save_quantized(const std::string& filename, float scale) {
...
void NNUE_512::init(const Yolah& yolah, Accumulator& a) {
...
void NNUE_512::play(uint8_t player, const Move& m, Accumulator& a) {
...
void NNUE_512::undo(uint8_t player, const Move& m, Accumulator& a) {
    int from = 63 - m.from_sq();
    int to = 63 - m.to_sq();
    // black positions + white positions + empty positions
    int pos = (player == Yolah::BLACK) ? 0 : 64;    
...
std::pair<float, float> NNUE_512::minmax_weights() const {
...
std::pair<float, float> NNUE_512::percentile_weights(float percentile) const {
...
void NNUE_512::get_activations(Accumulator& a, std::vector<float>& activations) {
    float h1[H1_SIZE];
    float h2[H2_SIZE];
    float h3[H3_SIZE];
    float output[OUTPUT_SIZE];
...
std::pair<float, float> NNUE_512::minmax_activations(const std::string& filename) {
    std::ifstream ifs(filename, std::ifstream::in);
...
std::pair<float, float> NNUE_512::percentile_activations(const std::string& filename, float percentile) {
    std::ifstream ifs(filename, std::ifstream::in);
    std::regex re_moves(R"(((\w\d):(\w\d))+)", std::regex_constants::ECMAScript);
    Accumulator acc = make_accumulator();
    std::vector<float> activations;
