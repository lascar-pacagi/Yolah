...
#
# Certain-win reasoning is unchanged from cnn_resnet_with_diff_score.py.
...
# ── Dataset ────────────────────────────────────────────────────────────────────
# Backed by the memory-mapped files produced by preprocess.py. This script
# reads all 6 planes (with certain-win); the ablation variant slices to 4.
IN_CHANNELS = 6
...
class GameDataset(Dataset):
    ...
    Each sample returns:
        state         : (IN_CHANNELS, 8, 8) float32 — full 6-plane encoding
...
class Net(nn.Module):
    ...
    ResNet for Yolah with TWO heads (AlphaZero style):
      ...
      Trunk : (B, 6, 8, 8) ──► input conv ──► N × ResBlock ──► BN ──► ReLU
...
NB_EPOCHS  = 3
MODEL_PATH = "/mnt/"
MODEL_NAME = "cnn_resnet_256x30_value_policy_with_diff_score"
