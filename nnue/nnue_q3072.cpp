...
static inline void matvec_16x3072(const int8_t* __restrict__ a, const int8_t* __restrict__ b, int8_t* __restrict__ c, 
...
static inline void matvec_16x32(const int8_t* __restrict__ a, const int8_t* __restrict__ b, int8_t* __restrict__ c,
                                const int16_t* __restrict__ bias) {    
    constexpr int register_width = 256 / 8;    
    constexpr int m = 16;
    constexpr int n = 32;
...
NNUE_Q3072::Accumulator NNUE_Q3072::make_accumulator() const {
    Accumulator a;
...
std::tuple<float, float, float> NNUE_Q3072::output(Accumulator& a) {        
...
void NNUE_Q3072::load(const std::string& filename) {
...
void NNUE_Q3072::init(const Yolah& yolah, Accumulator& a) {
    for (int i = 0; i < H1_SIZE; i++) {
        a.acc[i] = h1_bias[i];       
    }
...
void NNUE_Q3072::play(uint8_t player, const Move& m, Accumulator& a) {
...
void NNUE_Q3072::undo(uint8_t player, const Move& m, Accumulator& a) {
    int from = 63 - m.from_sq();
    int to = 63 - m.to_sq();
    // black positions + white positions + empty positions
    int pos = (player == Yolah::BLACK) ? 0 : 64;    
