#include "nnue_q1024x64x32x3_8bits2.h"
#include <fstream>
#include <string>
#include <regex>
#include "game.h"
...
static inline void matvec_64x1024(const int8_t* __restrict__ a, const int8_t* __restrict__ b, int8_t* __restrict__ c, 
    ...
    for (int i = 0; i < num_out_chunks; ++i) {
        ...
        const __m256i tmp = _mm256_cvtepi8_epi32(_mm_loadl_epi64((__m128i*)&bias[i * 8]));
        const __m256i _bias = _mm256_mullo_epi32(tmp, factor);
...
static inline void matvec_32x64(const int8_t* __restrict__ a, const int8_t* __restrict__ b, int8_t* __restrict__ c,
                                const int8_t* __restrict__ bias) {    
    constexpr int register_width = 256 / 8;    
    constexpr int m = 32;
    constexpr int n = 64;
...
static inline void matvec_3x32(const int8_t* __restrict__ a, const int8_t* __restrict__ b, int32_t* __restrict__ c,
                                const int8_t* __restrict__ bias) {
    constexpr int register_width = 256 / 8;    
    constexpr int n = 32;
    constexpr int num_in_chunks = n / register_width;
    ...
    const __m128i tmp = _mm_cvtepi8_epi32(_mm_loadl_epi64((__m128i*)&bias));
    const __m128i _bias = _mm_mullo_epi32(tmp, factor);
    __m128i outval = m256_haddx4(sum0, sum1, sum2, sum3, _bias);
    outval = _mm_srai_epi32(outval, SHIFT);
    _mm_store_si128((__m128i*)c, outval);
}

NNUE_Q1024x64x32x3_8bits2::Accumulator NNUE_Q1024x64x32x3_8bits2::make_accumulator() const {
    Accumulator a;
...
std::tuple<float, float, float> NNUE_Q1024x64x32x3_8bits2::output(Accumulator& a) {        
    ...
    //std::cout << std::endl;
    matvec_64x1024(h1_to_h2, h1, h2, h2_bias);
    for (int i = 0; i < H2_SIZE; i++) {
        std::cout << (int)h2[i] << ' ';
    }
    std::cout << std::endl;
    matvec_32x64(h2_to_h3, h2, h3, h3_bias);
    ...
    for (int i = 0; i < OUTPUT_SIZE; i++) {
        ...
        for (int j = 0; j < H3_SIZE; j++) {
            std::cout << (int)h3_to_output[i * 32 + j] << ' ';
            sum += (int32_t)h3_to_output[i * 32 + j] * (int32_t)h3[i];
        }
        std::cout << std::endl;
        sum += output_bias[i] * 64;
        sum >>= SHIFT;
        output[i] = sum;
    }
    for (int i = 0; i < OUTPUT_SIZE; i++) {
...
void NNUE_Q1024x64x32x3_8bits2::init(const Yolah& yolah, Accumulator& a) {
    for (int i = 0; i < H1_SIZE; i++) {
        a.acc[i] = h1_bias[i];       
    }
...
void NNUE_Q1024x64x32x3_8bits2::play(uint8_t player, const Move& m, Accumulator& a) {
...
void NNUE_Q1024x64x32x3_8bits2::undo(uint8_t player, const Move& m, Accumulator& a) {
    int from = 63 - m.from_sq();
    int to = 63 - m.to_sq();
    // black positions + white positions + empty positions
    int pos = (player == Yolah::BLACK) ? 0 : 64;    
...
int main(int argc, char* argv[]) {
    using namespace std;
    NNUE_Q1024x64x32x3_8bits2 nnue;
    try {
        nnue.load("nnue_q_1024x64x32x3_2.txt");
    } catch (const char* e) {
        cout << e << endl;
        exit(EXIT_FAILURE);
    }    
