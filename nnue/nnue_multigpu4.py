...
class GameDataset(Dataset):
    ...
    def encode_yolah(yolah):
        ...
        return np.nonzero(one_hot)[0].astype(np.uint8)
...
def custom_collate(batch, vocab_size=INPUT_SIZE):
    ...
    for i, sparse_indices in enumerate(inputs):
        ...
        indices = sparse_indices.astype(np.long)
        dense[i, indices] = 1.0

    # Convert targets to torch tensor, pin to memory, and move to device
    targets_tensor = torch.tensor(targets, dtype=torch.long, pin_memory=True).to(device)
    return dense, targets_tensor
