...
static void matvec_768x160(const float* __restrict__ a, const float* __restrict__ b, float* __restrict__ c) {    
    vec8 sum[20]{};
    constexpr int m = 768;
    constexpr int n = 160;
    for (int i = 0; i < m; i++) {
        vec8 bb = vec8{} + b[i];
        const vec8* aa = (vec8*)&a[i * n];
        for (int k = 0; k < 20; k++) {
            sum[k] += aa[k] * bb;
        }
    }
...
static inline void matvec_160x64(const float* __restrict__ a, const float* __restrict__ b, float* __restrict__ c) {    
    vec8 sum[8]{};
    constexpr int m = 160;
    constexpr int n = 64;
    for (int i = 0; i < m; i++) {
...
static inline void matvec_3x64(const float* __restrict__ a, const float* __restrict__ b, float* __restrict__ c) {
    constexpr int m = 3;
    constexpr int n = 64;
    for (int i = 0; i < m; i++) {
        float sum = 0;
...
NNUE_768x160x64x3::NNUE_768x160x64x3() {
...
NNUE_768x160x64x3::Accumulator NNUE_768x160x64x3::make_accumulator() const {
    Accumulator a;
...
std::tuple<float, float, float> NNUE_768x160x64x3::output(Accumulator& a) {    
    float h1[H1_SIZE];
    float h2[H2_SIZE];
    float h3[H3_SIZE];
    ...
    //std::cout << '\n' << "#################\n";
    matvec_768x160(weights_and_biases + H1_TO_H2, h1, h2);
    ...
    // }
    // std::cout << '\n' << "#################\n";
    matvec_160x64(weights_and_biases + H2_TO_H3, h2, h3);
    ...
    matvec_3x64(weights_and_biases + H3_TO_OUTPUT, h3, output);
...
void NNUE_768x160x64x3::load(const std::string& filename) {
...
void NNUE_768x160x64x3::save_quantized(const std::string& filename, float scale) {
...
void NNUE_768x160x64x3::init(const Yolah& yolah, Accumulator& a) {
...
void NNUE_768x160x64x3::play(uint8_t player, const Move& m, Accumulator& a) {
...
void NNUE_768x160x64x3::undo(uint8_t player, const Move& m, Accumulator& a) {
    int from = 63 - m.from_sq();
    int to = 63 - m.to_sq();
    // black positions + white positions + empty positions
    int pos = (player == Yolah::BLACK) ? 0 : 64;    
...
std::pair<float, float> NNUE_768x160x64x3::minmax_weights() const {
...
std::pair<float, float> NNUE_768x160x64x3::percentile_weights(float percentile) const {
...
void NNUE_768x160x64x3::get_activations(Accumulator& a, std::vector<float>& activations) {
    float h1[H1_SIZE];
    float h2[H2_SIZE];
    float h3[H3_SIZE];
    float output[OUTPUT_SIZE];
...
std::pair<float, float> NNUE_768x160x64x3::minmax_activations(const std::string& filename) {
    std::ifstream ifs(filename, std::ifstream::in);
...
std::pair<float, float> NNUE_768x160x64x3::percentile_activations(const std::string& filename, float percentile) {
    std::ifstream ifs(filename, std::ifstream::in);
    std::regex re_moves(R"(((\w\d):(\w\d))+)", std::regex_constants::ECMAScript);
    Accumulator acc = make_accumulator();
    std::vector<float> activations;
...
int main(int argc, char* argv[]) {
    using namespace std;
    NNUE_768x160x64x3 nnue;
    nnue.load("nnue_768x160x64x3.5.txt");
    nnue.save_quantized("nnue_q_768x160x64x3.5.txt", 64);
    //return 0;
