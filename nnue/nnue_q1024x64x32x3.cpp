...
static inline void matvec_64x1024(const int16_t* __restrict__ a, const int16_t* __restrict__ b, int16_t* __restrict__ c, 
...
NNUE_Q1024x64x32x3::Accumulator NNUE_Q1024x64x32x3::make_accumulator() const {
    Accumulator a;
...
std::tuple<float, float, float> NNUE_Q1024x64x32x3::output(Accumulator& a) {        
    ...
    std::cout << std::endl;
    matvec_64x1024(h1_to_h2, h1_16, h2_16, h2_bias);    
...
void NNUE_Q1024x64x32x3::load(const std::string& filename) {
...
void NNUE_Q1024x64x32x3::init(const Yolah& yolah, Accumulator& a) {
    for (int i = 0; i < H1_SIZE; i++) {
        a.acc[i] = h1_bias[i];       
    }
...
void NNUE_Q1024x64x32x3::play(uint8_t player, const Move& m, Accumulator& a) {
...
void NNUE_Q1024x64x32x3::undo(uint8_t player, const Move& m, Accumulator& a) {
    int from = 63 - m.from_sq();
    int to = 63 - m.to_sq();
    // black positions + white positions + empty positions
    int pos = (player == Yolah::BLACK) ? 0 : 64;    
