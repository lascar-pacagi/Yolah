...
NNUE_Q6::Accumulator NNUE_Q6::make_accumulator() const {
    Accumulator a;
...
std::tuple<float, float, float> NNUE_Q6::output(Accumulator& a) {        
    int32_t h2[H2_SIZE];
    int32_t h3[H3_SIZE];
    int32_t output[OUTPUT_SIZE];
    int16_t h1[H1_SIZE];
    int16_t h2_8[H2_SIZE];
    int16_t h3_8[H3_SIZE];
    for (int i = 0; i < H1_SIZE; i++) {
        int32_t v = a.acc[i];
        //std::cout << v << ' ';
...
void NNUE_Q6::load(const std::string& filename) {
...
void NNUE_Q6::init(const Yolah& yolah, Accumulator& a) {
    for (int i = 0; i < H1_SIZE; i++) {
        a.acc[i] = h1_bias[i];       
    }
...
void NNUE_Q6::play(uint8_t player, const Move& m, Accumulator& a) {
...
void NNUE_Q6::undo(uint8_t player, const Move& m, Accumulator& a) {
    int from = 63 - m.from_sq();
    int to = 63 - m.to_sq();
    // black positions + white positions + empty positions
    int pos = (player == Yolah::BLACK) ? 0 : 64;    
