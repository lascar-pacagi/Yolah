...
\title{
    ...
    \vspace{0.5cm}
    \Large Building a Two-Player Perfect-Information Game with Artificial Players\\ 
}
...
We have done our best with our current knowledge (\emph{ars longa, vita brevis}) to implement our game and the associated AIs. But like any good scientist, you should look at our work with a critical eye.
...
We will now describe the rules of the game, then we will explain why we chose these rules, we will give an example of a game between two AIs and then we will present the rest of the book.
...
In the next chapter, we will study the implementation of the Yolah game in C++ \cite{cppreference}. This implementation is designed to be efficient because it will be important for the AIs to be able to play many games per second; their level of play will depend on it.\\
...
\item Finally, on lines 29 to 42, if we found a \mintinline{c++}{MAGIC} constant that creates magic perfect hashing, we display it and then display a C++ array containing the position of the 1-bit for each bitboard. The output of this program for a given execution is shown below. 
...
\item At lines 6 to 9, the \mintinline{c++}{begin} and \mintinline{c++}{end} functions allow us to iterate over a \mintinline{c++}{MoveList} object. For example, we can write:
...
\item The remaining functions allow us to retrieve the size (\mintinline{c++}{size} at line 10), index the list like an array (\mintinline{c++}{operator[]} at lines 11 to 14), and directly obtain the pointer to the beginning of the \mintinline{c++}{move_list} array (\mintinline{c++}{data} at line 15).
...
\item Aux lignes 4 et 5, nous initialisons un générateur pseudo-aléatoire, soit avec une graine donnée par l'utilisateur (paramètre \mintinline{c++}{seed}) pour pouvoir reproduire le test, ou un nombre aléatoire en utilisant \mintinline{c++}{random_device}\footnote{Ce générateur va générer des nombres aléatoires de haute qualité, en utilisant par exemple {\ttfamily /dev/urandom} pour Linux. Par contre, l'obtention du nombre aléatoire va être assez lente comparée à l'utilisation d'un générateur pseudo-aléatoire comme \mintinline{c++}{mt19937} par exemple. Un des usages typique de \mintinline{c++}{random_device} est d'initialiser un générateur pseudo-aléatoire comme nous le faisons ici.}.

\item La boucle des lignes 5 à 24 va jouer \mintinline{c++}{nb_games} parties aléatoires.
\begin{itemize}
\item À la ligne 6, on initialise une nouvelle partie.
\item Aux lignes 7 à 22, on joue la partie aléatoire.
\begin{itemize}
\item Aux lignes 8 et 9, on récupère la liste des coups possibles dans la position actuelle du jeu et on les tries pour rendre l'affichage des coups possibles plus lisible.
\item Aux lignes 10 à 13, on affiche le nombre de coups possibles puis la liste de tous les coups. Vous pouvez voir le résultat dans le listing \ref{lst:random_games_testing_exec}. 
\item À la ligne 15, on affiche le plateau avec les coups représentés par des croix. Cela permet de voir facilement si des coups manquent ou si des coups sont de trop.
\item Aux lignes 16 à 18, on choisit un coup aléatoirement et on l'affiche.
\item Aux lignes 19 à 20, on attend l'appuie sur la touche entrée pour continuer la boucle. Cela nous laisse le temps d'analyser les sorties.
...
\begin{itemize}
\item Aux lignes 8 à 12, la structure \mintinline{c++}{TestResult} va contenir le résultat d'un test donné. L'attribut \mintinline{c++}{passed} est à vrai lorsque le test a réussi et à faux sinon. L'attribut \mintinline{c++}{message} stocke le diagnostic qui sera utilisé en cas d'erreur (voir la fonction \mintinline{c++}{run_test} à la ligne 80).

\item À la ligne 13, la fonction \mintinline{c++}{pass} rend un \mintinline{c++}{TestResult} qui indique que le test c'est déroulé correctement.

\item À la ligne 14, la fonction \mintinline{c++}{fail(string msg)} rend un \mintinline{c++}{TestResult} qui indique que le test a échoué avec le diagnostique d'erreur \mintinline{c++}{msg}.

\item Aux lignes 16 à 23, la fonction \mintinline{c++}{check_move_count} est notre premier test différentiel. Le premier paramètre \mintinline{c++}{fast} est la liste des coups obtenue grâce aux bitboards magiques, et le deuxième paramètre \mintinline{c++}{expected} est la liste de coups obtenue avec une méthode simple qui sera décrite plus loin et qui est définie aux lignes 87 à 114. Nous vérifions dans ce test que les nombres de coups produits par les deux approches sont bien identiques.

\item Aux lignes 25 à 53, la fonction \mintinline{c++}{check_move_lists_equal} est notre deuxième et dernier test différentiel. On vérifie que les listes de coups sont identiques, en les triant tout d'abord aux lignes 28 et 29, puis en vérifiant que les deux listes de coups sont maintenant identiques à la ligne 30. Le reste de la fonction crée un message d'erreur en calculant les différences entre les deux listes.

\item Aux lignes 55 à 61, la fonction \mintinline{c++}{check_undo} va vérifier la propriété suivante. 
...
\begin{minted}[breaklines,fontsize=\small,frame=none]{c++}
    ...
    Yolah after = yolah;
\end{minted}
On vérifie que l'annulation d'un coup fonctionne correctement en testant si \mintinline{c++}{before == after} à la ligne 56.

\item La deuxième propriété que l'on va tester est donnée dans la fonction \mintinline{c++}{check_game}\\\mintinline{c++}{_over_move} à la ligne 63. On y teste que le seul coup possible dans une position terminale du jeu est le coup \mintinline{c++}{Move::none()}. En effet, les joueurs n'ont plus de coups à disposition et ils doivent donc passer leur tour. À la ligne 65, on vérifie qu'il n'y a qu'un seul coup et que celui-ci est bien le coup \mintinline{c++}{Move::none()}.

\item Les dernières propriétés sont définies dans la fonction de test principale \mintinline{c++}{random_games} définie à la ligne 74. Ces dernières propriétés sont placées au sein de cette fonction, car elles ont besoin d'accéder aux attributs privés de la classe \mintinline{c++}{Yolah} et \mintinline{c++}{random_games} est déclarée \mintinline{c++}{friend} de cette classe.
\begin{itemize}
\item Aux lignes 87 à 114 est définie la function \mintinline{c++}{slow_moves_generation} qui permet d'obtenir la liste des coups possibles dans la configuration du jeu \mintinline{c++}{yolah} passée en paramètre (ligne 87). Comme nous le disions, cette façon de générer les coups est bien plus simple que celle à base de bitboards magiques. Nous avons une grande confiance dans l'exactitude de \mintinline{c++}{slow_moves_generation}. Le paramètre \mintinline{c++}{expected} dans \mintinline{c++}{check_move_lists_equal} à la ligne 25, a été instancié par une liste de coups obtenue grâce à \mintinline{c++}{slow_move_generation}. Les boucles aux lignes 91 et 92, vont parcourir toutes les cases du plateau. On construit la case de départ du coup \mintinline{c++}{from} à la ligne 93, puis on vérifie à la ligne 95 que cette case contient un pion du joueur pour lequel on crée la liste de coups (ligne 90). Les boucles aux lignes 96 et 97 vont parcourir toutes les directions possibles pour la pièce, on fait attention à filtrer le déplacement nul à la ligne 98. Enfin, la boucle de la ligne 100 à 107 va se déplacer dans la direction choisie \mintinline{c++}{(di, dj)} tant qu'on ne sort pas du plateau et qu'il n'y a pas d'obstacles. Si on sort du plateau à la ligne 101, ou qu'un obstacle est détecté à la ligne 104, on sort de la boucle pour essayer une nouvelle direction. On n'oublie pas à la ligne 112 d'ajouter le coup \mintinline{c++}{Move::none()} si aucun coup n'était possible. Cette façon de construire tous les coups possibles est très intuitive, mais inefficace.
\end{itemize}

\item À la ligne 116, nous allons tester la propriété que le coup \mintinline{c++}{Move::none()} doit seulement incrémenter le nombre de coups dans la partie grâce à la fonction \mintinline{c++}{check_none_move_execution}.

\item À la ligne 127, la fonction \mintinline{c++}{check_regular_move_execution} va tester les propriétés que doit vérifier un coup normal. Aux lignes 134 et 136, on vérifie que la pièce n'est plus sur la case de départ du coup, qu'elle se trouve maintenant sur la case d'arrivée du coup, et enfin qu'un trou est apparu sur la case de départ du coup.

\item Les lignes 136 à 188 déroulent \mintinline{c++}{nb_games} parties aléatoires en exécutant tous les tests.

\item Les lignes 190 à 202 affichent un résumé des tests. Le listing \ref{lst:random_games_property_based_testing_exec} donne le résultat de l'exécution du \mintinline{c++}{main}.
...
void random_games(size_t nb_games, optional<uint64_t> seed) {
  ...
  random_device rd; 
  mt19937 mt(seed.value_or(rd()));
  size_t total_tests = 0, passed_tests = 0;

  auto run_test = [&](TestResult result) {
    total_tests++;
    if (result) { passed_tests++; return true; }
  ...
  auto check_none_move_execution = [](const Yolah& before, 
                                      const Yolah& after) {
    if (before.black != after.black || before.white != after.white ||
        before.holes != after.holes || 
        before.black_score != after.black_score ||
  ...
  auto check_regular_move_execution = [](const Yolah& before, 
                                         const Yolah& after,
                                         Move m) {
    uint64_t player_bb = before.current_player() == BLACK ?
                         after.black : after.white;
    uint64_t from_bb = square_bb(m.from_sq());
    uint64_t to_bb = square_bb(m.to_sq());
    if (!(player_bb & from_bb) && (player_bb & to_bb) 
        && (after.holes & from_bb)) {
      return pass();
    }
    ostringstream oss;
    ...
    if ((player_bb & from_bb) != 0) {
        oss << "  From square should be cleared\n";
    }
    if ((player_bb & to_bb) == 0) {
        oss << "  To square should be set\n";
    }
    if ((after.holes & from_bb) == 0) {
        oss << "  From square should be a hole\n";
    }
    return fail(oss.str());
  };
  ...
  for (size_t i = 0; i < nb_games; i++) {
    ...
    while (!yolah.game_over()) {
      ...
      if (!run_test(check_move_count(fast_moves, 
                                     expected_moves))) break;
      if (!run_test(check_move_lists_equal(fast_moves, 
                                           expected_moves, 
                                           yolah))) break;
      ...
      yolah.play(m);
      if (m == Move::none()) {
        if (!run_test(check_none_move_execution(before, 
                                                yolah))) break;
      } else {
        if (!run_test(check_regular_move_execution(before, 
                                                   yolah, 
                                                   m))) break;
      }
      yolah.undo(m);
