...
Le résultat de cette commande est présenté dans le listing \ref{lst:perf_chapter02_1}.
\begin{itemize}
\item L'option \texttt{-r 10} de la commande \texttt{perf} ci-dessus permet de lancer le programme dix fois, pour faire la moyenne et calculer l'écart-type\footnote{Plus précisément, l'écart-type en pourcentage de la moyenne.} des résultats obtenus.
\item Le compteur \texttt{cycles} indique le nombre de cycles effectués par le processeur durant l'exécution de notre programme\footnote{En mode utilisateur et en mode noyau. Les compteurs sont activés quand un processus s'exécute et désactivés lorsqu'il perd la main.}.
