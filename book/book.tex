...
\item Aux lignes 17 à 44 est définie la function \mintinline{c++}{slow_moves_generation} qui permet d'obtenir la liste des coups possibles dans la configuration du jeu \mintinline{c++}{yolah} passée en paramètre (ligne 87). Cette fonction va nous permettre de faire les tests différentiels. Comme nous le disions, cette façon de générer les coups est bien plus simple que celle à base de bitboards magiques. Nous avons une grande confiance dans l'exactitude de \mintinline{c++}{slow_moves_generation}. Les boucles aux lignes 19 et 20, vont parcourir toutes les cases du plateau. On construit la case de départ du coup \mintinline{c++}{from} à la ligne 21, puis on vérifie à la ligne 22 que cette case contient un pion du joueur pour lequel on crée la liste de coups. Les fonctions \mintinline{c++}{square_of} et \mintinline{c++}{get_from} sont définies précisément dans le listing \ref{lst:chapter02}, mais \mintinline{c++}{Square square_of(int rank, int file)} va créer une case à partir des coordonnées \mintinline{c++}{(rank, file)} et \mintinline{c++}{uint8_t Yolah::get_from(Square sq)} retourne le type d'élément (un des joueurs, un trou ou un emplacement libre) sur la case \mintinline{c++}{sq}.
...
\item Aux lignes 158 à 234, la fonction \mintinline{c++}{random_games} va lancer tous les tests sur \mintinline{c++}{nb_games} parties aléatoires aux lignes 178 à 219. 221 à 233 Les lignes 190 à 202 affichent un résumé des tests. 
...
Nous allons analyser les performances de notre implémentation de Yolah en utilisant le programme du listing \ref{lst:random_games_perf1} qui ressemble au programme du listing \ref{fig:lst:random_games_testing}. Mais contrairement à ce dernier, le programme du listing \ref{fig:random_games_perf1} n'arrête pas le déroulement des parties et affiche quelques statistiques sur les parties.
\begin{itemize}
\item À la ligne 5, \mintinline{c++}{black_wins} va comptabiliser le nombre de victoire du joueur noir tout au long des \mintinline{c++}{nb_games} parties. À ligne 27, on affiche le nombre de victoire du joueur noir et le pourcentage que cela représente par rapport à toutes les parties jouées.
\item À la ligne 6, \mintinline{c++}{white_wins} va comptabiliser le nombre de victoire du joueur blanc. On affiche à la ligne 28 les statistiques associées.
\item À la ligne 7, on procède de même pour le nombre de parties nulles grâce à la variable \mintinline{c++}{draws}.
\item Aux lignes 10 à 15, on joue une partie aléatoire, en obtenant la liste des coups disponibles à la ligne 11, puis en sélectionnant un coup aléatoire à la ligne 13 puis en jouant ce coup à la ligne 14.
\item Nous allons jouer un million de parties comme nous pouvons le voir dans le \mintinline{c++}{main} à la ligne 34.
\end{itemize}
...
Performance counter stats for './chapter02' (100 runs):

17 671 802 014      cycles                    #    4,046 GHz                       ( +-  0,09% )  (49,84%)
24 345 746 995      instructions              #    1,37  insn per cycle            ( +-  0,01% )  (62,38%)
 2 431 465 524      branches                  #  556,680 M/sec                     ( +-  0,01% )  (62,44%)
   275 869 723      branch-misses             #   11,35% of all branches           ( +-  0,02% )  (62,54%)
 3 133 561 281      L1-dcache-loads           #  717,424 M/sec                     ( +-  0,01% )  (62,63%)
   146 564 424      L1-dcache-load-misses     #    4,68% of all L1-dcache accesses ( +-  0,07% )  (62,63%)
    54 670 067      LLC-loads                 #   12,517 M/sec                     ( +-  0,27% )  (50,00%)
        77 427      LLC-load-misses           #    0,14% of all LL-cache accesses  ( +- 22,57% )  (49,91%)

         4,37002 +- 0,00821 seconds time elapsed  ( +-  0,19% )
\end{verbatim}
}
\end{mdframed}
%\vspace{-0.5\baselineskip}
...
play_random_games                           95.17%
    ...
    │       └── pop_lsb                     11.93%
    │           └── lsb                     11.68%
    ├── std::uniform_int_distribution<...    8.61%
    ├── Yolah::play                          5.87%
    └── Yolah::game_over                     5.02%
        └── shift<>                          2.67%
\end{verbatim}
}
\end{mdframed}
...
12 832 947 780      cycles                    #    4,021 GHz                       ( +-  0,18% )  (49,84%)
23 589 523 540      instructions              #    1,82  insn per cycle            ( +-  0,01% )  (62,38%)
 2 328 267 849      branches                  #  729,484 M/sec                     ( +-  0,01% )  (62,40%)
   236 977 678      branch-misses             #   10,17% of all branches           ( +-  0,01% )  (62,53%)
 3 513 433 988      L1-dcache-loads           #    1,101 G/sec                     ( +-  0,01% )  (62,66%)
   145 286 847      L1-dcache-load-misses     #    4,13% of all L1-dcache accesses ( +-  0,07% )  (62,68%)
    54 964 849      LLC-loads                 #   17,221 M/sec                     ( +-  0,29% )  (50,01%)
        38 307      LLC-load-misses           #    0,07% of all LL-cache accesses  ( +- 12,03% )  (49,88%)

         3,19340 +- 0,00975 seconds time elapsed  ( +-  0,31% )
\end{verbatim}
}
\end{mdframed}
%\vspace{-0.5\baselineskip}
...
{\small
\begin{verbatim}
play_random_games_fast                     96.01%
    ├── Yolah::random_move                 81.29%
    │   ├── moves_bb                       47.14%
    │   │   └── Magic::index                5.99%
    │   ├── std::uniform_int_distributi... 12.88%
    │   ├── pop_lsb                         4.36%
    │   │   └── lsb                         4.11%
    │   └── std::popcount                   2.79%
    ├── Yolah::game_over                    9.53%
    │   └── shift<>                         5.96%
    └── Yolah::play                         3.95%
\end{verbatim}
}
...
=== Game Statistics ===
Total games: 1000000
Black wins:  498503 (49.9%)
White wins:  395975 (39.6%)
Draws:       105522 (10.6%)

Performance counter stats for './chapter02' (100 runs):

 6 446 405 052      cycles                    #    4,085 GHz                      ( +-  0,10% )  (49,63%)
15 466 860 279      instructions              #    2,41  insn per cycle           ( +-  0,01% )  (62,28%)
   793 399 286      branches                  #  502,748 M/sec                    ( +-  0,01% )  (62,53%)
     2 206 340      branch-misses             #    0,28% of all branches          ( +-  0,07% )  (62,79%)
 3 605 550 330      L1-dcache-loads           #    2,285 G/sec                    ( +-  0,01% )  (62,83%)
   145 697 914      L1-dcache-load-misses     #    4,03% of all L1-dcache accesses  ( +-  0,04% )  (62,75%)
    55 012 701      LLC-loads                 #   34,860 M/sec                    ( +-  0,26% )  (49,86%)
        11 412      LLC-load-misses           #    0,02% of all LL-cache accesses  ( +- 16,80% )  (49,60%)

         1,57951 +- 0,00265 seconds time elapsed  ( +-  0,17% )
\end{verbatim}
}
\end{mdframed}
%\vspace{-0.5\baselineskip}
