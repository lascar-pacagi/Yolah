...
\newacronym{z3}{Z3}{Z3 Theorem Prover - An efficient SMT solver developed by Microsoft Research}
...
\newglossaryentry{magicbitboard}{
    name=magic bitboard,
    description={A perfect hashing technique using multiplication and bit shifting to efficiently map occupied squares to move lookup tables}
}
...
Nous allons nous inspirer de l'excellent moteur de jeu d'échecs Stockfish \cite{stockfish2025} pour les structures de données de Yolah.

\section{Structure de données}

Nous allons représenter la position des noirs, des blancs et des cases détruites par des des entiers non signés sur $64$ bits~: \mintinline{c++}{uint64_t}. Nous appelerons bitboard ces entiers. Nous prenons des \mintinline{c++}{uint64_t} car le plateau de Yolah contient $64$ cases, nous avons donc un bit par case. La table \ref{tab:bitboard_indices} donne la position de chaque cases du plateau dans le bitboard. Cette information est représentée dans le code par l'énumération du listing \ref{lst:squares} \footnote{Par défaut, l'énumeration commence à la valeur 0, donc \mintinline{c++}{SQ_A1} vaut 0, \mintinline{c++}{SQ_A1} vaut 1, \ldots}.\\
...
\begin{itemize}
\item Les positions des pièces noires sur le plateau (table \ref{tab:black_positions_game_example_move21}) sont représentées par le bitboard~:
...
Pour tester la fin de partie nous allons avoir besoin de quelques énumerations et constantes (listings \ref{lst:directions} et \ref{lst:files_and_ranks}) et de la function \mintinline{c++}{shift} (listing \ref{lst:shift}). La constante \mintinline{c++}{NORTH} de l'énumeration \mintinline{c++}{Direction} (listing \ref{lst:directions}) est égale à $8$. Pourquoi avoir choisi cette valeur~? Reprenons ci-dessous, voir la table \ref{tab:bitboard_indices2}, la table vu au chapitre précédent représentant la position de chaque bit d'un bitboard dans le plateau. On peut voir que si l'on ajoute $8$ au numéro de bit contenu dans une des cases, on obtient le numéro de bit de la case au nord ce cette dernière (sauf si l'on sort du plateau de jeu). Il en va de même pour les autres valeurs des constantes de l'énumération \mintinline{c++}{Direction} (listing \ref{lst:directions}). 
\begin{table}[htpb]
\centering
...
La fonction \mintinline{c++}{shift(uint64_t b)} donnée dans le listing \ref{lst:shift} nous permet de décaler tous les bits à 1 du bitboard en paramètre dans la direction \mintinline{c++}{D} \footnote{Grâce aux \mintinline{c++}{if constexpr}, lorsque l'on effectuera \mintinline{c++}{shift<NORTH>(b)} par exemple, le compilateur transformera le code en~: \mintinline{c++}{return b << NORTH;}.}. Soit,
\begin{minted}[breaklines,fontsize=\small,frame=none]{cpp}
black == 1000000000000000000000000000100000100000000000000000000000000001
\end{minted}
le bitboard représenté dans la table \ref{tab:shift_north_before}. Pour décaler les pièces noires d'une case vers le nord, on effectue~: \mintinline{c++}{shift<NORTH>(black)}. On obtient alors le bitboard
  \begin{minted}[breaklines,fontsize=\small,frame=none]{cpp}
1000000000000000000000000000100000100000000000000000000000000001
\end{minted}
représenté dans la table \ref{tab:shift_north_after}. Notons que la pièce en {\ttfamily h8} n'est plus sur le plateau.\\

\begin{table}[H]
\centering
\begin{minipage}{0.45\textwidth}
\centering
\caption{Plateau de jeu avant d'effectuer \mintinline{c++}{shift<NORTH>}}
...
\begin{minipage}{0.45\textwidth}
\centering
\caption{Plateau de jeu après avoir effectuer \mintinline{c++}{shift<NORTH>}}
...
On peut ainsi facilement obtenir toutes les cases directement au contact des pièces d'un plateau donné \mintinline{c++}{board}, en effectuant l'opération suivante~:
...
Pour le plateau de la table \ref{tab:before_all_shift} on obtient les positions représentées par des étoiles dans la table \ref{tab:after_all_shift}.\\
...
6 & $\cdot$ & $\cdot$ & $\cdot$ & $\cdot$ & $\cdot$ & $\cdot$ & $\cdot$ & $\cdot$ \\ \hline
5 & $\cdot$ & $\cdot$ & $\cdot$ & $\times$ & $\circ$ & $\cdot$ & $\cdot$ & $\cdot$ \\ \hline
4 & $\cdot$ & $\cdot$ & $\cdot$ & $\times$ & $\times$ & $\bullet$ & $\cdot$ & $\cdot$ \\ \hline
3 & $\cdot$ & $\cdot$ & $\cdot$ & $\cdot$ & $\cdot$ & $\cdot$ & $\cdot$ & $\cdot$ \\ \hline
...
Dans le code de la fonction \mintinline{c++}{shift} (listing \ref{lst:shift}) on peut voir l'utilisation des constantes \mintinline{c++}{FileHBB} et \mintinline{c++}{FileABB} ({\ttfamily BB} pour bitboard). Ces constantes vont nous permettre de masquer certains bits qui se retrouveraient après décalage dans une position incorrecte. Par exemple, si nous décalons dans la direction \mintinline{c++}{EAST} le bitboard représenté dans la table \ref{tab:before_all_shift}, le pion blanc en {\ttfamily h1} se retrouverait en {\ttfamily a2}. Pour éviter cela, avant le décalage, on élimine les éléments de la colonne {\ttfamily h} pour qu'ils ne se retrouvent pas dans la colonne {\ttfamily a} (ligne 8 du listing \ref{lst:shift}). Les constantes \mintinline{c++}{FileABB} et \mintinline{c++}{FileHBB} ainsi que les énumerations pour les rangées et colonnes sont définies dans le listing \ref{lst:files_and_ranks}.    
...
La fonction qui teste la fin de partie s'appelle \mintinline{c++}{game_over} et elle est décrite dans le listing \ref{lst:game_over}. Son implémentation est assez simple,

\begin{itemize}

\item À la ligne 2, on récupère dans \mintinline{c++}{possible} le bitboard où il y a des $1$ dans les positions libres.

\item À la ligne 3, on crée le bitboard \mintinline{c++}{players} qui contient des $1$ dans les positions occupées par l'un des joueurs.

\item Les lignes 4 à 8 permettent de créer le bitboard \mintinline{c++}{around_players} qui contient des $1$ aux positions autour de chacune des pièces des joueurs.

\item Enfin, à la ligne 8, on teste s'il n'existe aucune position libre à côté d'un des joueurs. En effet, le et bit-à-bit ne gardera un $1$ dans un bit du résultat, que si et seulement s'il y a un $1$ à cette position dans le bitboard
\mintinline{c++}{possible} et dans le bitboard \mintinline{c++}{around_players}. Donc que la case correspondante est libre et accessible par le joueur. Si le résultat de \mintinline{c++}{around_players & possible} vaut $0$, cela veut dire qu'aucune position n'est à la fois à côté d'un des joueurs et libre.
...
La génération des coups possibles va utiliser une technique appelée les \emph{magic bitboard}. Pour comprendre cette technique je vais tout d'abord la présenter sur un exemple simple. Je me suis inspiré de l'article \url{http://pradu.us/old/Nov27_2008/Buzz/research/magic/Bitboards.pdf} pour cet exemple.

\subsection{Hachage parfait}

Supposons que nous devions trouver la position du seul bit à $1$ dans un entier long non-signé (\mintinline{c++}{uint64_t}). Par exemple, si nous appelons \mintinline{c++}{position} la fonction permettant de calculer cette position, nous obtenons les résultats suivants~:
...
Notons que les processeurs possèdent des instructions permettant de calculer efficacement cette fonction \footnote{\mintinline{asm}{tzcnt} sous Intel.}, et le compilateur {\ttfamily gcc} permet de calculer efficacement cette position avec la fonction \mintinline{c++}{__builtin_ctzll(unsigned long long x)} \footnote{Returns the number of trailing 0-bits in x, starting at the least significant bit position. If x is 0, the result is undefined.}, par exemple,

\begin{minted}[breaklines,fontsize=\small,frame=none]{cpp}
    position(0b1000) == __builtin_ctzll(0b1000)
\end{minted}

Mais nous allons coder cette fonction \mintinline{c++}{position} en utilisant un hachage parfait afin d'illustrer cette technique, qui nous servira pour la génération des coups possibles par la suite.\\
...
La consultation de cette table de hachage est bien plus coûteuse que l'indexation d'un simple tableau \footnote{Nous mesurerons à la fin de cette section le gain apporté par le hachage parfait par rapport à l'utilisation d'une table de hachage classique.}. Mais on ne peut pas directement utiliser un tableau, car les valeurs pour indexer celui-ci sont dans une plage de valeurs trop grande, par exemple la valeur \mintinline{c++}{0x8000000000000000} ($9,223,372,036,854,775,808$~!). L'idée c'est de trouver une fonction de hachage efficace pour transformer chacune des $64$ valeurs\footnote{\mintinline{c++}{1, 0b10, 0b100, 0b1000, 0b10000, ...}.}, que nous allons appeler clés, dans la plage $[0, 2^6 - 1 = 63]$.
De plus on veut qu'il n'y ait aucune collision, d'où le nom de hachage parfait. S'il y a des collisions des positions seraient erronées. Par exemple, si les clés \mintinline{c++}{0b100} et \mintinline{c++}{0b1000000} étaient transformées en l'indice $42$, on devrait mettre dans \mintinline{c++}{table[42]} la valeur $2$ ou $6$, mais on ne peut pas stocker plus d'une valeur au même emplacement.\\
...
constexpr int K = 6;
int perfect_hashing(uint64_t key) {
    return key * MAGIC >> (64 - K);
}
\end{minted}
...
\item À la ligne 2, la constante \mintinline{c++}{K} donne le nombre de bits pour notre index. Ici avec \mintinline{c++}{K == 6}, cela nous donne un maximum de $2^K = 2^6 = 64$ valeurs possibles dans la table.

\item À la ligne 4, on peut voir la formule pour calculer l'index dans la table selon la clé \mintinline{c++}{uint64_t key} donnée en paramètre. On multiplie la clé par la constante \mintinline{c++}{MAGIC} et ensuite on décale vers la droite pour ne garder que les \mintinline{c++}{K} bits de poids forts du résultat. Cette opération est très peu coûteuse, mais comment trouver cette constante magique~?\\

\end{itemize}

Une méthode simple pour essayer de trouver la constante \mintinline{c++}{MAGIC} est de la générer au hasard jusqu'à trouver une valeur qui ne crée aucune collision~! Ce genre d'approche est utilisée dans Stockfish \cite{stockfish2025} pour générer les magic bitboard. Le listing \ref{lst:random_magic} montre cette approche.
...
\end{minted}
\captionof{listing}{Recherche aléatoire de la constante \mintinline{c++}{MAGIC}}
\label{lst:random_magic}
\end{mdframed}

\begin{itemize}

\item Aux lignes 6 à 8, nous mettons en place un générateur aléatoire pour générer aléatoirement des \mintinline{c++}{uint64_t}.

\item À la ligne 9, la table de correspondance \mintinline{c++}{positions} va associer à un entier long ne contenant qu'un seul bit à $1$, la position de ce dernier. Cette table est initialisée aux lignes 10 à 12.

\item À la ligne 13, la constante \mintinline{c++}{K} est le nombre de bits de notre clé obtenue par hachage parfait. Il faut que \mintinline{c++}{K} permettent de représenter toutes les valeurs que l'on doit couvrir. Par exemple, ici nous avons $64$ valeurs possibles, car nous avons $64$ positions possibles pour le bit à $1$ dans un entier long sur $64$ bits. Avec \mintinline{c++}{K == 7}, on peut couvrir $2^7$, soit $128$ valeurs différentes. Notons que c'est plus que les $64$ valeurs dont nous avons besoin et que \mintinline{c++}{K == 6} suffirait (\mintinline{c++}{K == 5} serait trop petit). Mais en essayant de trouver la constante \mintinline{c++}{MAGIC} au hasard pour \mintinline{c++}{K == 6}, ce programme ne trouvait pas en un temps raisonnable \frownie{} (nous verrons après une autre façon de faire pour réussir pour \mintinline{c++}{K == 6} \smiley).

\item Aux lignes 14 à 16, la fonction \mintinline{c++}{index} calcule l'index de clé \mintinline{c++}{bitboard} (ce bitboard contient un seul bit à $1$) en utilisant la formule pour notre hachage parfait.

\item La boucle \mintinline{c++}{while}, lignes 17 à 43, va boucler jusqu'à trouver la constante \mintinline{c++}{MAGIC} qui permet de réaliser un hachage parfait (du coup cette boucle peut être infinie).

\item À la ligne 18, on crée la valeur \mintinline{c++}{MAGIC} au hasard.

\item Le \mintinline{c++}{set<uint64_t>} de la ligne 20 va nous permettre de mémoriser les index (obtenues grâce à la fonction \mintinline{c++}{index}) que nous avons déjà utilisés pour vérifier que nous n'avons pas de collisions.

\item Aux lignes 21 à 28, la boucle \mintinline{c++}{for} va tester tous les bitboards, trouver l'index de chacun d'entre eux grâce au hachage (fonction \mintinline{c++}{index}) et vérifier que nous n'avons pas de collision (ligne 23) avec les index obtenus pour les bitboard précédents.

\item Enfin, aux lignes 29 à 42, si nous avons trouvé une constante \mintinline{c++}{MAGIC} qui permet de créer le hachage parfait, nous affichons celle-ci puis nous affichons un tableau en \emph{C++} qui contient la position du bit à $1$ pour chacun des bitboard. La sortie de ce programme, pour une exécution donnée, est donnée ci-dessous. 
...
Pour obtenir la position du bit à $1$ pour le bitboard \mintinline{c++}{0b1000} par exemple, il faut donc consulter la valeur suivante~:
...
On remarque qu'il y a des places vacantes dans le tableau \mintinline{c++}{positions}, celles contenant la valeur $-1$, donc on a perdu de la place. Nous n'aurions gaspillé aucune place si nous avions trouvé une constante 
\mintinline{c++}{MAGIC} pour \mintinline{c++}{K == 6}. Mais est-ce possible~?

\end{itemize}

Pour répondre à cette question nous allons procéder d'une autre manière. Nous ne voulons pas, bien entendu, balayer les $2^{64}$ ($9, 223, 372, 036, 854, 775, 808$) valeurs possibles puis à chaque fois tester si la valeur permet le hachage parfait. Nous allons utiliser un solveur \gls{smt} qui va nous permettre de fixer des contraintes avant de balayer tout l'espace de recherche. Le fait de fixer tout d'abord des contraintes va permettre d'élaguer l'espace de recherche. Notons que nous n'avons pas la garantie que la recherche soit rapide, mais pour ce problème, l'obtention d'une constante \mintinline{c++}{MAGIC} pour \mintinline{c++}{K == 6} sera très rapide. Si vous êtes intéressé par le fonctionnement d'un solveur \gls{smt} vous pouvez consulter \cite{kroening2016decision} et \cite{biere2021handbook}.\\

Le code du listing \ref{lst:z3_magic} décrit l'approche utilisant le solveur \gls{z3} \cite{moura2008z3}.
...
if solver.check() == sat:
    ...
    print('\n};')
\end{minted}
\captionof{listing}{Recherche avec un solveur \gls{smt} de la constante \mintinline{c++}{MAGIC}}
\label{lst:z3_magic}
\end{mdframed}

\begin{itemize}

\item Les lignes 3 à 5 permettent de faire correspondre à un bitboard donné ne contenant qu'un bit à $1$, la position correspondante de ce bit. On utilise pour ce faire le dictionnaire \mintinline{python}{positions} (ligne 3).

\item À la ligne 7, on instantie le solveur \gls{z3}.

\item À la ligne 8, on déclare que la constante \mintinline{python}{MAGIC} est du type \mintinline{python}{z3.BitVec} qui est un type proposé par le solveur \gls{z3} qui permet de représenter des vecteurs de bits, ici nous utilisons un vecteur de $64$ bits.

\item La fonction \mintinline{python}{index} définie aux lignes 12 et 13 permet de calculer le hachage en fonction de la constante \mintinline{python}{MAGIC} et de \mintinline{python}{K}.

\item Aux lignes 15 à 19 on donne au solver les contraintes que les valeurs hachées par la fonction \mintinline{python}{index} de deux bitboards différents ne doivent pas se retrouver au même emplacement. Cette contrainte permet d'assurer que le hachage est parfait.

\item À la ligne 21 on execute le solveur et il nous indique par la valeur \mintinline{python}{sat} s'il a trouvé une valeur pour \mintinline{python}{MAGIC} qui satisfait les contraintes. Notons que si le solveur répond \mintinline{python}{unsat} à la place de \mintinline{python}{sat}, cela veut dire qu'il n'existe aucun entier sur $64$ bits qui permette le hachage parfait. Même si l'espace de recherche est très grand, sur ma machine, l'obtention d'une solution se fait en moins d'une seconde~!

\item Les lignes 22 à 32 permettent d'afficher sur la sortie standard la valeur \mintinline{python}{MAGIC} ainsi que le tableau \emph{C++} des valeurs obtenues. Ce tableau est donné ci-dessous. Notons qu'en \emph{Python} il était important d'effectuer 
l'opération \mintinline{python}{index(m, K, bitboard) & size - 1} à la ligne 28 car les nombres entiers ont une précision arbitraire et par exemple nous avons
...
Remarquons que nous n'avons pas le même problème à la ligne 18 car \mintinline{python}{MAGIC} est de type \mintinline{python}{BitVec(64)}.\\
...
Maintenant que nous avons vu sur un exemple simple comment marche la technique des magic bitboard (ou hachage parfait), nous allons pouvoir étudier la génération des mouvements possibles des pièces en utilisant cette technique. 

\subsection{Magic bitboard pour les mouvements des pièces}

\subsection{Jouer et annuler un coup}

\section{Code complet commenté du plateau de jeu}

\begin{mdframed}[skipabove=\baselineskip,hidealllines=true]
\inputminted[linenos,breaklines,fontsize=\small]{cpp}{code/type.h}
\captionof{listing}{Squares, files, ranks and directions}
\label{lst:type_h}
\end{mdframed}
...
\captionof{listing}{implémentation d'un coup}
\label{lst:move_h}
\end{mdframed}
...
\captionof{listing}{implémentation du plateau}
\label{lst:game_h}
\end{mdframed}
