...
MinMaxPlayerV11::MinMaxPlayerV11(uint64_t microseconds, size_t tt_size_mb, size_t nb_threads, heuristic_eval h) 
    : thinking_time(microseconds), table(tt_size_mb),
      pool(static_cast<BS::concurrency_t>(nb_threads)), heuristic(h) {
}

Move MinMaxPlayerV11::play(Yolah yolah) {
    this->stop = false;
...
std::string MinMaxPlayerV11::info() {
    return "minmax player (transposition table + pvs + killer + lazy SMP)";
}

int16_t MinMaxPlayerV11::negamax(Yolah& yolah, Search& s, uint64_t hash, int16_t alpha, int16_t beta, int8_t depth) {
    ++s.nb_nodes;
    if (yolah.game_over()) {
    ...
    if (depth <= 0) {
        int16_t v = heuristic(yolah.current_player(), yolah);
    ...
    for (size_t i = 0; i < moves.size(); i++) {        
        ...
        yolah.play(m);
        if (full_window) {            
            v = -negamax(yolah, s, zobrist::update(hash, player, m), -beta, -alpha, depth - 1);            
        } else {
            v = -negamax(yolah, s, zobrist::update(hash, player, m), -alpha - 1, -alpha, depth - 1);
            if (v > alpha) v = -negamax(yolah, s, zobrist::update(hash, player, m), -beta, -alpha, depth - 1);
        }
        yolah.undo(m);        
        if (v >= beta) {
...
int16_t MinMaxPlayerV11::root_search(Yolah& yolah, Search& s, uint64_t hash, int8_t depth, Move& res) {
    res = Move::none();
    Yolah::MoveList moves;
    yolah.moves(moves);
    int16_t alpha = -std::numeric_limits<int16_t>::max() + 10;
    int16_t beta  = std::numeric_limits<int16_t>::max() - 10;
    sort_moves(yolah, s, hash, moves);
    auto player = yolah.current_player();
    bool full_window = true;
    for (size_t i = 0; i < moves.size(); i++) {     
...
void MinMaxPlayerV11::sort_moves(Yolah& yolah, const Search& s, uint64_t hash, Yolah::MoveList& moves) {
...
void MinMaxPlayerV11::print_pv(Yolah yolah, uint64_t hash, int8_t depth) {
...
void MinMaxPlayerV11::iterative_deepening(Yolah yolah, Search& s) {    
...
json MinMaxPlayerV11::config() {
    json j;
    j["name"] = "MinMaxPlayerV11";
