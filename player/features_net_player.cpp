...
int16_t FeaturesNetPlayer::root_search(Yolah& yolah, Search& s, uint64_t hash, int8_t depth, Move& res) {
    res = Move::none();
    Yolah::MoveList moves;
    yolah.moves(moves);
...
void FeaturesNetPlayer::sort_moves(Yolah& yolah, const Search& s, uint64_t hash, Yolah::MoveList& moves) {
...
void FeaturesNetPlayer::print_pv(Yolah yolah, uint64_t hash, int8_t depth) {
...
void FeaturesNetPlayer::iterative_deepening(Yolah yolah, Search& s) {    
...
json FeaturesNetPlayer::config() {
    json j;
    j["name"] = "FeaturesNetPlayer";
