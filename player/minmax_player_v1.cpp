...
std::string MinMaxPlayerV1::info() {
    return "minmax player v1";
}

int32_t MinMaxPlayerV1::negamax(Yolah& yolah, uint64_t hash, int32_t alpha, int32_t beta, uint8_t depth) {
    nb_nodes++;
    if (yolah.game_over()) {
...
int32_t MinMaxPlayerV1::root_search(Yolah& yolah, uint64_t hash, uint8_t depth, Move& res) {
    res = Move::none();
    Yolah::MoveList moves;
    yolah.moves(moves);
    ...
    for (const Move& m : moves) {        
        yolah.play(m);
        int32_t v = -negamax(yolah, zobrist::update(hash, player, m), alpha, beta, depth - 1);
        yolah.undo(m);
        if (v > alpha) {
            alpha = v;
            res = m;
