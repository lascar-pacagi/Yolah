...
MinMaxNNUE_QuantizedPlayer::MinMaxNNUE_QuantizedPlayer(uint64_t microseconds, size_t tt_size_mb, size_t nb_moves_at_full_depth, 
                                                        uint8_t late_move_reduction, const std::string& nnue_q_parameters_filename, size_t nb_threads) 
    : thinking_time(microseconds), table(tt_size_mb),
      ...
      nnue_q_parameters_filename(nnue_q_parameters_filename),
      pool(static_cast<BS::concurrency_t>(nb_threads)) {
        nnue.load(nnue_q_parameters_filename);
    }

Move MinMaxNNUE_QuantizedPlayer::play(Yolah yolah) {
    this->stop = false;
...
std::string MinMaxNNUE_QuantizedPlayer::info() {
    return "minmax nnue quantized player (transposition table + late move reduction + killer + lazy SMP)";
}

int16_t MinMaxNNUE_QuantizedPlayer::negamax(Yolah& yolah, Search& s, uint64_t hash, int16_t alpha, int16_t beta, int8_t depth) {
    ++s.nb_nodes;
    if (yolah.game_over()) {
...
int16_t MinMaxNNUE_QuantizedPlayer::root_search(Yolah& yolah, Search& s, uint64_t hash, int8_t depth, Move& res) {
    res = Move::none();
    Yolah::MoveList moves;
    yolah.moves(moves);
...
void MinMaxNNUE_QuantizedPlayer::sort_moves(Yolah& yolah, const Search& s, uint64_t hash, Yolah::MoveList& moves) {
...
void MinMaxNNUE_QuantizedPlayer::print_pv(Yolah yolah, uint64_t hash, int8_t depth) {
...
void MinMaxNNUE_QuantizedPlayer::iterative_deepening(Yolah yolah, Search& s) {    
...
json MinMaxNNUE_QuantizedPlayer::config() {
    json j;
    j["name"] = "MinMaxNNUE_QuantizedPlayer";
