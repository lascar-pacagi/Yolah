...
FeatureNetPlayer::FeatureNetPlayer(
...
Move FeatureNetPlayer::play(Yolah yolah) {
    this->stop = false;
...
std::string FeatureNetPlayer::info() {
    return "feature net player (transposition table + late move reduction + killer + lazy SMP)";
}

int16_t FeatureNetPlayer::negamax(Yolah& yolah, Search& s, uint64_t hash, int16_t alpha, int16_t beta, int8_t depth) {
    ++s.nb_nodes;
    if (yolah.game_over()) {
...
int16_t FeatureNetPlayer::root_search(Yolah& yolah, Search& s, uint64_t hash, int8_t depth, Move& res) {
    res = Move::none();
    Yolah::MoveList moves;
    yolah.moves(moves);
...
void FeatureNetPlayer::sort_moves(Yolah& yolah, const Search& s, uint64_t hash, Yolah::MoveList& moves) {
...
void FeatureNetPlayer::print_pv(Yolah yolah, uint64_t hash, int8_t depth) {
...
void FeatureNetPlayer::iterative_deepening(Yolah yolah, Search& s) {    
...
json FeatureNetPlayer::config() {
    json j;
    j["name"] = "FeatureNetPlayer";
